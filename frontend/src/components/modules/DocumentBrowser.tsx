...
function PreviewModal({
  ...
  // <img>/<iframe> can't send the JWT header, so fetch the file with auth
  // headers and serve it from a local object URL (keeps the token out of URLs).
  useEffect(() => {
    ...
    setLoadError(false);
    setObjectUrl(null);
    fetch(previewUrl, { headers: ChatAPI.getDocumentPreviewHeaders() })
      .then((res) => {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {!loadError && isPdf(file.mime, file.name) && (
            <iframe
              src={src}
          ...
          {!loadError && isImage(file.mime, file.name) && (
          ...
          {!loadError && !isPdf(file.mime, file.name) && !isImage(file.mime, file.name) && (
